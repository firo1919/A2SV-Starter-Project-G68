	...
	return (
		<div className="p-3 bg-white shadow-xl rounded-md mx-auto w-full max-w-full sm:max-w-lg md:max-w-[800px] h-auto">
			...
			{stagesToShow.map((stage, index) => {
				...
				// - If backend status is "under_review" (1), then submitted is completed, under_review is current, others are future
				...
				return (
					<div key={stage.id} className="flex gap-6 items-start mb-6 flex-col md:flex-row">
						<img
							src={stageInfo.icon}
							alt={stage.alt}
							className={`w-10 h-10 object-cover rounded-md ${isFutureStage ? "opacity-50" : ""}`}
						/>
						<div>
							<h2
							...
							{!isFutureStage && <p className="text-sm text-gray-500">{stageInfo.description}</p>}
