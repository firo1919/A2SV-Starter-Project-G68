...
export default function DashboardClient({ initialApplicationStatus, initialProfileData }: DashboardClientProps) {
	...
	const hasSubmittedApplication =
		initialApplicationStatus?.success && initialApplicationStatus.data && initialApplicationStatus.data.id;

	// Extract application status for Card component
					...
					<div className="order-1 lg:order-1 lg:col-span-2">
						...
						<div>
							<div className="bg-gradient-to-r to-[#9333EA] from-[#4F46E5] p-4 sm:p-6 lg:p-8 rounded-xl shadow-lg mb-6 sm:mb-8">
								...
								>
									{hasSubmittedApplication ? "View Application" : "Start Application"}
								</button>
							</div>
						</div>
					</div>
					<div className="order-2 lg:order-2">
						<Card type="dashboard" applicationStatus={applicationStatus} />
					</div>
				</div>
			</div>
		</main>
