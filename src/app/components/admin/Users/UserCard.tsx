...
const UserCard = ({id, profile, username, user_email, role, status, onDelete}: UserType) => {
    ...
    const handleEdit = () => {
        router.push(`/admin/update-user?id=${id}`);
    };
  return (
    <div className='grid grid-cols-2 md:grid-cols-4 justify-between bg-white p-4 items-center gap-8 w-9/10 md:w-full'>
        ...
        <div className='flex justify-start w-full'>
            <h2 className={`px-2 py-0.5 w-fit rounded-2xl font-semibold ${status ? 'bg-[#DBEAFE] text-[#166534]' : 'bg-[#FEF9C3] text-[#854D0E]'}`}>{status ? "Active" : "Inactive"}</h2>
        </div>
        <div className='flex justify-end gap-6 w-full px-8'>
            <button onClick={handleEdit} className='font-medium bg-transparent text-[#4F46E5]'>Edit</button>
